    ...
    pages: [
      ...
      },
      {
        ...
        steps: [
          ...
          '「社内工数」タブで据付・試運転などの労務費も計上できる。支払条件は候補から選択（自由入力も可）。',
          '「検印（承認者）」を選んで「承認依頼を送る」→承認者が「承認する」と正式発行できます。承認までPDFには「draft」透かしが入ります（承認後に内容を保存し直すと承認は解除され再依頼が必要）。',
          '「CADから見積作成」でDXF図面をアップロードすると、型式を抽出して見積の骨格をdraftで自動生成できます（プロトタイプ）。',
          '保存後に「PDF出力」「受注票」発行（種別は案件の工番／単番区分に従う）「複製」が行える。',
        ],
        status: ['下書（下書き）', '承認待ち／承認済', '受注（採用済）', '受注済（受注票発行済）'],
        note: 'PDF出力は保存後のみ可能。受注票は案件子IDに工番／単番が設定されていないと発行できません。検印者はユーザー管理の機能権限「検印承認者」を持つ人から選べます。複製時は複製先の案件子IDの指定が必須です。受注票を発行するとステータスが「受注済」になります。CADからの自動生成はダクトが概算で、取付工費・運送費・工数は含まれないため必ず内容を確認してください。',
      },
      {
