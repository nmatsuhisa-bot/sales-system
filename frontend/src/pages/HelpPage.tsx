...
const PAGE_GROUPS = [
  {
    ...
    pages: [
      ...
      },
      {
        ...
        note: 'PDF出力は保存後のみ可能。受注票は案件子IDに工番／単番が設定されていないと発行できません。検印者はマスタ管理「従業員・ユーザー」で機能権限「検印承認者」を持つ人から選べます。複製時は複製先の案件子IDの指定が必須です。受注票を発行するとステータスが「受注済」になります。承認依頼メールが送信できない環境では、承認者はログイン後の画面上部の承認待ち通知から承認できます。案件子IDは見積書・社内工数試算・承認依頼メールにも表示されます。CADからの自動生成はダクトが概算で、取付工費・運送費・工数は含まれないため必ず内容を確認してください。',
      },
      {
    ...
    pages: [
      {
        ...
        note: '管理者（admin）アカウントは誤って無効化しないよう削除ボタンが表示されません。見積の承認機能を使うには機能権限「検印承認者」を、営業担当を選べるようにするには「営業担当」を持つユーザーを登録してください。旧「ユーザー管理」メニューはこのタブに統合されました（/users は自動でここへ移ります）。',
      },
      {
