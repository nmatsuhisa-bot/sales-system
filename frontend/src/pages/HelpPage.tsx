    ...
    pages: [
      {
        ...
        steps: [
          ...
          '発注書ごとにヘッダー（発注先・注文日・納入場所等）と明細（部材・数量・単価・納期）を編集。明細追加の部材検索は入力が止まってから候補を取得し、上位15件までを表示する。',
          '明細行の「入荷」「在庫引当」ボタンで在庫管理に反映される。1明細につきどちらか一度だけで、登録後は行に「入荷済」「在庫引当」と表示されボタンは出なくなる。発注書が「キャンセル」のときは「入荷」ボタンも出ず、入荷・在庫引当はできない。',
          '一覧の「入荷登録」は発注書内の未処理の明細をまとめて在庫に加算する（在庫引当済み・入荷済み・数量未設定の明細はスキップされ、その件数が完了メッセージに出る）。',
          '一覧の「発注書」ボタンで注文書（発注先・注文日・納入場所・製番・件名と明細）を別タブに表示して印刷できる。単価・金額は端数があるときだけ小数2桁で印字される。',
          '明細の数量は0より大きい数、単価は0以上（空欄可）で入力する。ヘッダーの注文日は空欄のままでも保存できる。',
          '発注を取り消すときは、入荷登録・在庫引当の記録がある発注書は削除できないため、ステータスを「キャンセル」に変更する（記録が無い発注書はこれまでどおり削除できる）。',
          '部材の台帳やユニット構成の管理は「製品BOMマスタ」画面に集約。',
        ],
        status: ['作成中', '発注済', '一部入荷', '入荷済', 'キャンセル'],
        note: '発注書は「作成中」の間だけヘッダー・明細を編集できます。ヘッダーや明細の保存・追加・削除、部材検索に失敗したときは、発注書の詳細エリア上部に赤いメッセージで理由が表示されます（表示は×で消せます）。同じ明細を二重に入荷・在庫引当しようとした場合や、キャンセルした発注書の明細を入荷・在庫引当しようとした場合もこのメッセージで拒否されるため、在庫が二重に計上されたり取り消した発注で在庫が動いたりすることはありません。一覧の「金額」は発注書詳細の「合計」と同じ値（端数があるときは小数2桁）で表示されます。旧「発注・仕入管理」画面（発注書一覧のみの簡易版）も残っていますが、実務ではこちらを使用してください。',
      },
      {
