...
const PAGE_GROUPS = [
  {
    ...
    pages: [
      ...
      },
      {
        ...
        status: ['工番（紫）', '単番（オレンジ）'],
        note: '種別は受注票発行時に合計300万円で自動判定されますが、この画面で手動変更もできます。見積を受注票として再発行すると、旧受注票は自動的に「過去」扱いとなり一覧から隠れます（「過去」フィルタで表示可能）。',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    pages: [
      ...
      },
      {
        ...
        note: '営業中・確度高・内示は案件の予算金額を採用（受注以降は見積金額）。これらは0円でも一覧に個別表示されます。合計金額300万円未満（単番）の案件は個別表示せず最下部に集計し、見積のない月も平均的な単番売上を「月次仮計上」として見込みます。',
      },
      {
