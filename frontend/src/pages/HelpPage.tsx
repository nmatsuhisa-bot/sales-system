    ...
    pages: [
      {
        ...
        steps: [
          ...
          '明細行の「入荷」「在庫引当」ボタンで在庫管理に反映される。1明細につきどちらか一度だけで、登録後は行に「入荷済」「在庫引当」と表示されボタンは出なくなる。',
        ...
        note: '発注書は「作成中」の間だけヘッダー・明細を編集できます。ヘッダーや明細の保存・追加・削除、部材検索に失敗したときは、発注書の詳細エリア上部に赤いメッセージで理由が表示されます（表示は×で消せます）。同じ明細を二重に入荷・在庫引当しようとした場合もこのメッセージで拒否されるため、在庫が二重に計上されることはありません。旧「発注・仕入管理」画面（発注書一覧のみの簡易版）も残っていますが、実務ではこちらを使用してください。',
      },
      {
