    ...
    pages: [
      ...
      },
      {
        ...
        steps: [
          ...
          '保存後に「PDF出力」「受注票」発行（税抜合計300万円以上は工番／未満は単番と自動判定）「複製」が行える。',
        ],
        status: ['下書（下書き）', '受注（採用済）', '受注済（受注票発行済）'],
        note: 'PDF出力は保存後のみ可能。複製時は複製先の案件子IDの指定が必須です。受注票を発行するとステータスが「受注済」になります。',
      },
      {
        ...
        steps: [
          ...
          '各行の鉛筆アイコン「受注項目の編集」で、種別（工番/単番の手動変更）・注文書の有無・納期・前受金（最大3回の分割入金）・出荷方法・受注日を登録・修正できる。',
          '各行の「PDF」アイコンで受注票PDFを表示。',
        ],
        status: ['工番（紫）', '単番（オレンジ）'],
        note: '種別は受注票発行時に税抜合計300万円で自動判定されますが、この画面で手動変更もできます。見積を受注票として再発行すると、旧受注票は自動的に「過去」扱いとなり一覧から隠れます（「過去」フィルタで表示可能）。',
      },
    ],
  },
  {
    ...
    pages: [
      ...
      },
      {
        ...
        steps: [
          ...
          '鍵アイコンからパスワード変更、編集アイコンから氏名・部門等の修正、不要なアカウントは無効化できる。',
        ],
      ...
      },
      {
        ...
        note: '営業中・確度高・内示は案件の予算金額を採用（受注以降は見積金額）。これらは0円でも一覧に個別表示されます。税抜合計金額300万円未満（単番）の案件は個別表示せず最下部に集計します。単番の集計は経過月（先月まで）が実数、今月以降は個別の見積が出揃わないため25M/月の見込みで計上します。',
      },
      {
